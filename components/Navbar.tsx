...
const Navbar = ({session}: {session: Session | null}) => {
  return (
    <nav className="bg-white shadow-md py-4 border-b
    border-gray-200">
        {""}
        ...
        flex items-center space-x-2">
          {session ? (
            <>
            <Link href={"/trips"} className="text-slate-900 hover:text-sky-500 px-4">My Trips</Link>
        <Link href={"/globe"} className="text-slate-900 hover:text-sky-500 px-4">Globe</Link>
          <button className="flex items-center bg-gray-800 hover:bg-gray-900 text-white p-2 rounded-sm cursor-pointer"
        onClick={logout}>
            Sign Out  
